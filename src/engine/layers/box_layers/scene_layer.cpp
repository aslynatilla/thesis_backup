    ...
    void SceneLayer::on_attach() {
        {
            ...
            point_light.position = glm::vec3(278.0f, 545.0f, 279.5f);
...
//                                                      "resources/shaders/depth.frag");
            ...
            gs_depth_shader = shader::create_shader_from("resources/shaders/point_sm.vert", "resources/shaders/point_sm.frag",
                                                         "resources/shaders/point_sm.gs");
...
//            glEnable(GL_CULL_FACE);
            glGenFramebuffers(1, &sm_framebuffer);

            glGenTextures(1, &cubemap);
            glBindTexture(GL_TEXTURE_CUBE_MAP, cubemap);
            for(auto i = 0; i < 6; ++i){
                glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X+i, 0,GL_DEPTH_COMPONENT,
                             800, 800, 0, GL_DEPTH_COMPONENT, GL_FLOAT, nullptr);
            }
            if(glGetError() != 0){
...
//        glViewport(0, 0, static_cast<unsigned int>(viewport_dimensions[2]), static_cast<unsigned int>(viewport_dimensions[3]));
        ...
        glm::mat4 proj;
        for(auto i = 0; i < 6; ++i){
            const auto point_light_ith_camera = Camera(CameraGeometricDefinition{pos, pos + OpenGL3_Cubemap::directions[i], OpenGL3_Cubemap::ups[i]},
                    90.0f, 1.0f,
                    CameraPlanes{0.1f, 1000.0f},
                    CameraMode::Perspective);
            light_transforms.push_back(point_light_ith_camera.get_view_matrix());
            proj = point_light_ith_camera.get_projection_matrix();
        }
        std::transform(std::begin(light_transforms), std::end(light_transforms), std::begin(light_transforms),
                       [&proj](auto& light_mat){ return proj * light_mat; });

        glBindFramebuffer(GL_FRAMEBUFFER, sm_framebuffer);
        glClear(GL_DEPTH_BUFFER_BIT);
        gs_depth_shader->use();
        gs_depth_shader->set_mat4("shadow_matrices[0]", light_transforms[0]);
        gs_depth_shader->set_mat4("shadow_matrices[1]", light_transforms[1]);
        gs_depth_shader->set_mat4("shadow_matrices[2]", light_transforms[2]);
        gs_depth_shader->set_mat4("shadow_matrices[3]", light_transforms[3]);
        gs_depth_shader->set_mat4("shadow_matrices[4]", light_transforms[4]);
        gs_depth_shader->set_mat4("shadow_matrices[5]", light_transforms[5]);
        gs_depth_shader->set_float("far_plane", 1000.0f);
        gs_depth_shader->set_vec3("light_position", pos);
        if (!scene_objects.empty()) {
            for (const auto& drawable : scene_objects) {
                gs_depth_shader->set_mat4("model", drawable.transform);
                OpenGL3_Renderer::draw(*(drawable.vao));
            }
        }
        glBindFramebuffer(GL_FRAMEBUFFER, 0);

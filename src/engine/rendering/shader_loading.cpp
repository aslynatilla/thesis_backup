...
namespace engine::shader{
    ...
    std::shared_ptr<Shader> create_shader_from(const std::string& vertex_path_as_string,
        ...
        return std::make_shared<Shader>(std::move(vertex_source), std::move(fragment_source),
    ...
    std::shared_ptr<Shader> create_shader_from(const std::string& vertex_path_as_string,
        ...
        return std::make_shared<Shader>(std::move(vertex_source), std::move(geometry_source), std::move(fragment_source),
                                        std::string_view(vertex_filename), std::string_view(geometry_filename), std::string_view(fragment_filename));
    }
}
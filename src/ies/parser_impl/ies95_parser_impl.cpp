#include "ies95_parser_impl.h"

namespace ies::parser_impl {
    bool IES95_ParserImpl::can_handle(const std::string_view& ies_data) {
        return ies_data.starts_with("IESNA:LM-63-1995");
    }

    std::optional<IES_Document>
    IES95_ParserImpl::handle(const std::string& filename, std::string&& ies_data) {
        using namespace ies::string_view_manipulation;

        const auto lines = data_as_lines(ies_data);
        IES_Standard parsed_standard = IES_Standard::IES_95;
    ...
    bool IES95_ParserImpl::no_more_entry_lines(const std::string_view& line) {
